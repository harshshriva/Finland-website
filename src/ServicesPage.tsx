...
const ServicesPage = () => {
  ...
  return (
    <div className="py-12 px-2 md:px-0 flex flex-col items-center fade-in-up">
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-8">
        {services &&
          services?.map((s, i) => (
            <div
              key={i}
              className="rounded-lg shadow p-4 flex flex-col items-center border border-gray-200 fade-in-up"
            ...
            >
              ...
              <h3 className="text-xl font-extrabold text-white mb-2 text-center uppercase">
                {s.title}
              </h3>
              <p className="text-white text-sm mb-2 text-center">{s.desc}</p>
              <Button
                type="primary"
                className="text-white w-full font-bold tracking-wider mt-auto"
